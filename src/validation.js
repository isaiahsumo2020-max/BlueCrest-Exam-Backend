...
export const auditIdSchema = z.object({ id }).strict()
...
export const studentSchema = z.object({ id, studentId: z.string().trim().min(1).max(100).optional(), rollNumber: z.string().trim().min(1).max(100).optional(), name: z.string().trim().min(1).max(200), email: z.string().trim().email().max(254), accessCode: z.string().trim().max(100).optional(), phone: z.string().trim().max(40), programmeId: id, currentSemester: z.number().int().positive().max(40), sessionId: id, status: z.enum(['active', 'inactive', 'graduated']), enrolledAt: date }).strict()
